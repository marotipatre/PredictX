...
type Feature = {
  ...
  icon: any; // `icon` can be a React element or component (e.g., an icon component)
  caption: string; // `caption` should be a string
  title: string; // `title` should be a string
  text: string; // `text` should be a string
  button: string; // `button` text should be a string (assuming it's a label or button title)
  url: string; // `url` should be a string (link URL)
};
      ...
      <div>
        <div className="container">
          <div className="relative flex md:flex-wrap flex-nowrap border-2 border-s3 rounded-7xl md:overflow-hidden max-md:flex-col feature-after md:g7 max-md:border-none max-md:rounded-none max-md:gap-3">
            {features.map(({ id, icon, caption, title, text, button,url }
            ) => (
              <div
                key={id}
              ...
              >
                ...
                <Button icon={button.icon}><a href={url}>{button.title}</a></Button>
              </div>
            ))}
